...
const QuestionActions = () => {
  ...
  const { setUpdateQuestionModal, setDeleteQuestionModal } = globalActions
  return (
    <Menu as="div" className="inline-block text-left text-[#BBBBBB]">
      ...
      <Menu.Items
        className="absolute right-0 mr-2 mt-2 w-56 origin-top-right
           divide-[#BBBBBB] rounded-md bg-[#16112E] shadow-md 
          ring-1 ring-black ring-opacity-5 focus:outline-none shadow-pink-900 "
      >
        <Menu.Item>
          {({ active }) => (
            <button
        ...
        <Menu.Item>
          {({ active }) => (
            <button
              ...
              onClick={() => dispatch(setDeleteQuestionModal('scale-100'))}
            >
              <BsTrash3 size={17} />
              <span>Delete</span>
            </button>
